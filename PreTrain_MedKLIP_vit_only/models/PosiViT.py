...
class VisionTransformer(nn.Module):
    ...
    def forward_head(self, x: torch.Tensor, pre_logits: bool = False) -> torch.Tensor:
        if self.attn_pool is not None:
            ...
            x = x[:, 0:self.num_prefix_tokens]  # class token
        x = self.fc_norm(x)
        x = self.head_drop(x)
...
#         if self.global_pool:
#             x = x[:, 2:, :].mean(dim=1)  # global pool without cls token
...
if __name__ == "__main__":
    # test
    model = vit_small_patch16_224(reg_tokens=0, pretrained=False, num_classes=768).to(
        torch.device("cuda")
    )  # PosiViT(reg_tokens=1).to(torch.device('cuda'))
...
#     fc2 = nn.Linear(num_ftrs, 256).to(torch.device("cuda"))
#     cls_fea = y[:, 0]
#     print(cls_fea.shape)
#     pos_fea = y[:, 1]
#     print(pos_fea.shape)
